#!/bin/bash
# KØREXIA-MD Heroku One-Click Deployer
# Usage: bash <(curl -s https://raw.githubusercontent.com/GlobalTechInfo/KØREXIA-MD/main/lib/heroku.sh)
...
echo -e "${CYAN}"
echo "╔═══════════════════════════════════════╗"
echo "║       KØREXIA-MD Heroku Deployer         ║"
echo "║         by Nsala             ║"
echo "╚═══════════════════════════════════════╝"
echo -e "${NC}"
...
# App name
read -p "$(echo -e ${CYAN}App name (e.g. my-korexia-md): ${NC})" APP_NAME
APP_NAME=${APP_NAME:-korexia-md-bot}

# Session ID
read -p "$(echo -e ${CYAN}Session ID (Nsala/KØREXIA-MD_xxxxx): ${NC})" SESSION_ID
...
# Owner number
read -p "$(echo -e ${CYAN}Owner WhatsApp number (e.g. 50931221850): ${NC})" OWNER_NUMBER
OWNER_NUMBER=${OWNER_NUMBER:-50931221850}

# Bot name
read -p "$(echo -e ${CYAN}Bot name (default: KØREXIA-MD): ${NC})" BOT_NAME
BOT_NAME=${BOT_NAME:-KØREXIA-MD}
...
if [ ! -f "heroku.yml" ]; then
    echo -e "${YELLOW}📦 Cloning KØREXIA-MD repo...${NC}"
    git clone https://github.com/GlobalTechInfo/MEGA-MD korexia-md-deploy
    cd korexia-md-deploy
fi
