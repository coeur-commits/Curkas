...
const ffmpegBuffer = (buffer, args = []) => {
...
export const generateProfilePicture = async (buffer) => {
    const output = await ffmpegBuffer(buffer, [
        '-vf',
        'crop=min(iw\\,ih):min(iw\\,ih):(iw-min(iw\\,ih))/2:(ih-min(iw\\,ih))/2,scale=720:720:force_original_aspect_ratio=decrease,pad=720:720:(ow-iw)/2:(oh-ih)/2',
        '-frames:v', '1',
        '-c:v', 'mjpeg',
        '-q:v', '3'
    ]);
...
export const reSize = async (buffer, ukur1, ukur2) => {
    const width = Number(ukur1);
    const height = Number(ukur2);
    ...
    return await ffmpegBuffer(buffer, [
        '-vf',
        `scale=${Math.max(1, Math.floor(width))}:${Math.max(1, Math.floor(height))}:force_original_aspect_ratio=decrease,pad=${Math.max(1, Math.floor(width))}:${Math.max(1, Math.floor(height))}:(ow-iw)/2:(oh-ih)/2`,
        '-frames:v', '1',
        '-c:v', 'mjpeg',
        '-q:v', '3'
    ]);
