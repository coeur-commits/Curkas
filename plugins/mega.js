...
export default {
    ...
    aliases: ['megadl'],
    category: 'download',
    description: 'Download from KØREXIA with real-time progress',
    usage: '.korexia <korexia-url>',
    async handler(sock, message, args, context) {
        const chatId = context.chatId || message.key.remoteJid;
        const text = args.join(' ').trim();
        if (!text) {
            return sock.sendMessage(chatId, { text: `*Usage:* .korexia https://korexia.nz/file/xxxx#xxxx` }, { quoted: message });
        }
        try {
