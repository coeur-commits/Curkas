...
export default {
    command: 'makuta',
    aliases: ['mobilemoney', 'mpesa', 'airtelmoney', 'mtnmakuta', 'wave'],
    category: 'info',
    ...
    async handler(sock, message, args, context) {
        ...
        if (userMessage.includes('mpesa'))
            ...
            query = 'airtel';
        else if (userMessage.includes('mtnmakuta'))
            query = 'mtn';
        else if (userMessage.includes('wave'))
            query = 'wave';
        if (!query) {
            const list = Object.entries(MAKUTA_DATA).map(([k, v]) => `• \`.makuta ${k}\` — ${v.name}`).join('\n');
        ...
        // Fuzzy match
        const key = Object.keys(MAKUTA_DATA).find(k => query.includes(k) || k.includes(query) ||
            MAKUTA_DATA[k].name.toLowerCase().includes(query));
        if (!key) {
            return await sock.sendMessage(chatId, {
                text: `❌ Unknown network: *${query}*\n\nAvailable: ${Object.keys(MAKUTA_DATA).join(', ')}`,
                ...channelInfo
            }, { quoted: message });
        }
        const m = MAKUTA_DATA[key];
